老板，您的审美要求非常在线！确实，一个专业的界面能极大地提升使用体验和公司形象。

我已经按照您的要求完成了全面的视觉升级：

1. **修复 Logo**：我把您提供的黑色线框 Logo (图二) 直接**嵌入到了代码里**（用 Base64 编码技术），这样无论网络状况如何，它都100%能显示出来，再也不会破图了。
2. **更名**：左上角的名称已改为大气的 **SEVEN O'CLOCK GROUP**。
3. **增加专业背景**：我挑选了一张**非常有质感的深色纺织纱线仓库**图片作为全局背景，并加上了暗色滤镜，既契合咱们的行业属性，又能保证前景文字清晰易读，非常有“国际贸易巨头”的范儿。

请回到 **GitHub**，编辑 **`app.py`**，**全选删除**原有代码，粘贴下面这段 **最终视觉升级版** 代码：
...
# === 视觉优化：注入专业的行业背景图 ===
# 我们使用 CSS 技术来设置全局背景，并添加暗色遮罩以确保文字清晰
page_bg_img = """
<style>
[data-testid="stAppViewContainer"] {
    /* 专业的纱线/纺织仓库背景图 (深色调) */
    background-image: url("https://images.unsplash.com/photo-1555697723-55f8974e093d?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D");
...
[data-testid="stAppViewContainer"]::before {
    ...
    width: 100%;
    height: 100%;
    background-color: rgba(0, 0, 0, 0.7); /* 70%透明度的黑色 */
    z-index: -1;
}
...
def analyze_cross_check(po_text, req_text, docs_text, mode, api_key):
    ...
    focus = {
        "信用证 (L/C)": "比对【单据】是否符合【信用证】及【合同】。",
        "托收 (CAD/DP)": "比对【单据】是否符合【银行指示】。",
        "电汇 (T/T)": "比对【单据】是否符合【合同】。"
    }.get(mode, "")
...
# === Tab 1: 单证·审核台 ===
with tab1:
    # 使用一个半透明的容器来包裹内容，让背景图透出来，但又不影响阅读
    with st.container():
        ...
        with col_info:
            if contract_no:
                st.success(f"归档命名预览：**{contract_no}_..._已审核.zip**")
            else:
                st.warning("👈 请输入合同号以启动 AI 审核引擎。")
        ...
        with c2:
            if mode == "电汇 (T/T)":
                st.info("T/T 模式：无需上传要求文件，AI 将重点比对合同与单据。")
                f_req = None
            else:
...
with tab2:
    ...
    with st.container():
        ...
        st.caption("提示：数据保存在当前浏览器会话中，刷新页面将重置。")
